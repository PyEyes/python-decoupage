...
def affiche_polygon(segments, couleur):
    """
    Affiche un polygon
    ...
    for segment in segments:
        for point in segment:
            chaine_coordonnes += str(round(point[0], 2))+","+str(round(point[1], 2))+","
...
def creer_tranche(hauteur, largeur, nom_fichier, segments_coupes, couleur):
    """
        Creation de la tranche
    """
    header = entete(hauteur, largeur)
    body = affiche_polygon(segments_coupes, couleur)
    end = fin()
